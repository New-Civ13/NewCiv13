/obj/map_metadata/nomads_gaia
	ID = MAP_NOMADS_GAIA
	title = "Nomads (Gaia)"
	lobby_icon = 'icons/lobby/gaia.png'
	...
	songs = list(
		"In the beginning there was nothing:1" = 'sound/music/in_the_beginning.ogg',)
	research_active = TRUE
	nomads = TRUE
	gamemode = "Classic (Stone Age Start)"
	var/list/arealist_r = list()
	var/list/arealist_g = list()
/obj/map_metadata/nomads_gaia/New()
	..()
	spawn(18000)
		seasons()

/obj/map_metadata/nomads_gaia/cross_message(faction)
	return ""

/obj/map_metadata/nomads_gaia/job_enabled_specialcheck(var/datum/job/J)
	if (J.is_nomad == TRUE)
		. = TRUE
	else
		. = FALSE
