...
/obj/structure/branch/attack_hand(mob/living/human/H)
	if (H.a_intent == I_GRAB && leaves)
		to_chat(H, SPAN_NOTICE("You start picking the leaves from \the [src]..."))
		if (do_after(H, 60, src))
			if (src && leaves)
				...
				to_chat(H, SPAN_NOTICE("You gain some leaves from the branch."))
				name = "cleared branch"
				desc = "A tree branch with all the leaves picked out."
				icon_state = "cleared_stick"
				leaves = FALSE
...
/obj/item/weapon/branch/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (W.edge && !sharpened)
		to_chat(user, SPAN_NOTICE("You start sharpening the stick..."))
		if (do_after(user, 80, src))
			if (src && !sharpened)
				to_chat(user, SPAN_NOTICE("You finish sharpening the stick."))
		...
		if (F.sharpened)
			...
			if (do_after(user, 100, src))
				if (src && F && F.sharpened)
					to_chat(user, SPAN_NOTICE("You finish making the flint hatchet."))
